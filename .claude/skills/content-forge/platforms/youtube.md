...
静态配图走本 skill 第六步：写 HTML 源 → `scripts/render.sh` 渲成 PNG。本配置只管
这里需要哪几类图、什么规格。

**要做的是成片而不是配图时，交给 `illustrated-videos` skill。** 它管风格预设、分镜、
素材生成、镜头编排和渲染，顺序和产物见那个 skill 的 `docs/pipeline.md`。
...
- **开篇先交代实验对象，再给认知**。前 30 秒说明这次在测什么、为什么测、输入和产出是什么；再给两到三条认知。不要把「片 A / 片 B / QA」当作观众已经知道的上下文，也不要用一条纯时间顺序的复述开场。
...
- Hook 在抛冲突前已交代实验对象；陌生观众能理解片名、QA 或返工指的是什么。
